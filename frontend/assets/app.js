...
let activeWorkspace = null, activeWorkspaceData = null, activeReadOnly = false, uploading = false;
...
function setMode(next) {
  ...
  const descriptions={user:'Sign in to your private workspaces.',supervisor:'Review team workspaces with read-only access.',administrator:'Manage every workspace and local account role.'};
...
(async()=>{setPortal(portal,false);try{await showWorkspace(await api('/auth/me'));}catch{showAuth();}})();
...
async function loadDocuments(id){
  const data=await api(`/workspaces/${id}/documents`);
  if(activeWorkspace!==id)return;
  const list=$('document-list');list.replaceChildren();
  const read=data.documents.filter(doc=>doc.extracted).length;$('metric-documents').textContent=data.documents.length;$('metric-read').textContent=`${read} ready for questions`;$('material-summary').textContent=`${data.documents.length} ${data.documents.length===1?'file':'files'} · ${read} read`;$('use-documents').disabled=activeReadOnly||read===0;if(read===0||activeReadOnly)$('use-documents').checked=false;$('source-readiness').textContent=activeReadOnly?'Supervisor review mode is read-only':read?`${read} read ${read===1?'material':'materials'} available for grounding`:'Read material to enable source-grounded answers';
  if(!data.documents.length){const empty=document.createElement('div');empty.className='compact-empty';empty.innerHTML='<span>▤</span><h3>No materials yet</h3><p>Choose files above to build this workspace’s local knowledge collection.</p>';list.append(empty);return;}
  for(const doc of data.documents){
    const row=document.createElement('div');row.className='document-row';row.dataset.name=doc.filename.toLowerCase();row.dataset.readiness=doc.extracted?'ready':'unread';
    ...
    const meta=document.createElement('p');meta.textContent=`${formatSize(doc.size)} · ${doc.extracted?'Ready for source questions':'Unread'}`;
    infoText.append(name,meta);info.append(type,infoText);const link=document.createElement('a');link.className='secondary';link.href=`/api/documents/${doc.id}/download`;link.textContent='Download';link.setAttribute('download',doc.filename);const actions=document.createElement('div');actions.className='file-actions';actions.append(link);const fileExt=(doc.filename.split('.').pop()||'').toLowerCase();const docActions=activeReadOnly?[[doc.extracted?'View sources':'Not yet read',()=>showSource(doc.id)]]:[[doc.extracted?'Read again':'Read locally',()=>readMaterial(doc.id,id)],['View sources',()=>showSource(doc.id)]];if(!activeReadOnly&&['pdf','png','jpg','jpeg'].includes(fileExt))docActions.push(['Describe image',()=>describeImage(doc.id,id)]);if(!activeReadOnly)docActions.push(['Create review pack',()=>startReview(doc.id,id)]);for(const [label,action] of docActions){const button=document.createElement('button');button.type='button';button.className='secondary';button.textContent=label;button.onclick=action;if(activeReadOnly&&!doc.extracted)button.disabled=true;actions.append(button);}if(!activeReadOnly){const remove=document.createElement('button');remove.type='button';remove.className='danger-button';remove.textContent='Remove';remove.onclick=()=>openDelete({kind:'document',id:doc.id,name:doc.filename});actions.append(remove);}row.append(info,actions);list.append(row);
  }
...
async function loadGenerations(id){
  ...
  for(const item of data.generations){
    ...
    const answer=document.createElement('pre');answer.className='answer-text';answer.textContent=item.answer;
...
$('prompt-form').addEventListener('submit',async event=>{
  ...
  try{
    await api(`/workspaces/${id}/${$('use-documents').checked?'questions':'generations'}`,{method:'POST',body:JSON.stringify({prompt})});
    if(activeWorkspace===id){$('generation-status').textContent='Answer saved in this workspace.';await loadGenerations(id);await loadModels();}
...
async function startReview(documentId,workspace){try{await api(`/workspaces/${workspace}/runs`,{method:'POST',body:JSON.stringify({document_id:documentId})});showTab('workflows');toast('Review workflow started.');await loadRuns(workspace);}catch(e){error('upload-error',e.message);}}
...
async function loadRuns(id){
 ...
 if(run.result.draft||run.result.code){const text=document.createElement('pre');text.className='answer-text';text.textContent=run.result.draft||run.result.code;card.append(text);}
...
setInterval(()=>{if(activeWorkspace&&!document.hidden)loadRuns(activeWorkspace);},2500);
